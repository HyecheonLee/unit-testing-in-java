package com.hyecheon.iloveyouboss.util;
...
public class SparseArray<T> {
    ...
    public T get(int key) {
        final int index = binarySearch(key, keys, size);
        if (index != -1 && keys[index] == key) {
    ...
    public void checkInvariants() {
        final long nonNullValues = Arrays.stream(values).filter(Objects::nonNull).count();
